...
class AdaptiveAMMSimulated(IContract):
    """
    ...
    async def _resolve(self, symbol, exchange, balance_percentage_to_use, 
                 max_order_balance_percentage, randomness_factor, 
                 volatility_threshold, max_order_size, min_order_size,
                 order_book, current_price, volume, open_orders,
                 balance) -> dict:
        """
        ...
        async with EquivalencePrinciple(
            ...
            task = f"""You are an adaptive market maker for the trading symbol '{symbol}' from '{exchange}' on a centralized exchange. 
            ...
            Market Data:
            - symbol: {symbol}
            - Order Book (bid and ask prices with volumes): {order_book}
            - Current Price: {current_price}
            - 24-hour Volume: {volume}
            - Open Orders: {open_orders}
            - Balance: {balance}
            - Balance Percentage to Use: {balance_percentage_to_use}  # % of total balance to allocate to market making
            - Max Order Balance Percentage: {max_order_balance_percentage}  # % of balance to allocate per individual order
            ...
            6. **Balance Constraints**: Ensure that the total allocation of orders does not exceed {balance_percentage_to_use * 100}% of 
              the available balance for each asset in {balance}. Additionally, each individual order should not exceed 
              {max_order_balance_percentage * 100}% of the total balance to manage risk.

            7. **Market Sentiment and News**: If there is relevant news or events impacting {symbol}, adjust the aggressiveness of order 
    ...
    async def resolve(self, symbol, exchange, balance_percentage_to_use, 
                 max_order_balance_percentage, randomness_factor, 
                 volatility_threshold, max_order_size, min_order_size,
                 order_book, current_price, volume, open_orders,
                 balance) -> None:
        ...
        """
        resolve_response = await self._resolve(symbol, exchange, balance_percentage_to_use, max_order_balance_percentage, randomness_factor,
                            volatility_threshold, max_order_size, min_order_size,
                            order_book, current_price, volume, open_orders, balance)
